...
static const city_coords_t cities[] = {
    {"Rosario", "-32.9468", "-60.6393"} // Única ciudad fija
};

// Weather data storage
static weather_info_t weather_data[LOCATION_NUM_MAX];
static geocoding_location_t user_location; // ubicación elegida por el usuario
...
esp_err_t http_event_handler(esp_http_client_event_t *evt)
{
    ...
    case HTTP_EVENT_ON_FINISH:
        ...
        if (output_buffer != NULL) {
            ESP_LOGI(TAG, "Datos de clima recibidos: %s", output_buffer);
...
esp_err_t app_weather_parse_openmeteo(const char *json_string, weather_location_t location)
{
    ...
    // Nombre de ubicación: si hay geocoding válido usar solo el nombre de la ciudad elegida, sino Rosario
    if (location == LOCATION_NUM_USER && user_location.is_valid) {
        snprintf(info->location, sizeof(info->location), "%s", user_location.city_name);
    } else {
        strncpy(info->location, cities[0].name, sizeof(info->location) - 1);
    }
    info->is_valid = true;
    
        ESP_LOGI(TAG, "Clima actualizado para %s: %d°C, %s", 
...
void app_weather_code_to_text(weather_type_code_t code, char *text, size_t text_size)
{
    ...
    switch (code) {
        case 0:
            strncpy(text, "Cielo despejado", text_size - 1);
            break;
        case 1: case 2: case 3:
            strncpy(text, "Parcialmente nublado", text_size - 1);
            break;
        case 45: case 48:
            strncpy(text, "Niebla", text_size - 1);
            break;
        case 51: case 53: case 55:
            strncpy(text, "Llovizna", text_size - 1);
            break;
        case 61: case 63: case 65:
            strncpy(text, "Lluvia", text_size - 1);
            break;
        case 71: case 73: case 75:
            strncpy(text, "Nieve", text_size - 1);
            break;
        case 80: case 81: case 82:
            strncpy(text, "Chubascos", text_size - 1);
            break;
        case 95: case 96: case 99:
            strncpy(text, "Tormenta eléctrica", text_size - 1);
            break;
        default:
            strncpy(text, "Desconocido", text_size - 1);
            break;
    }
    text[text_size - 1] = '\0';
}
...
esp_err_t app_weather_request(weather_location_t location)
{
    ...
    if (location == LOCATION_NUM_USER && user_location.is_valid) {
        // Usar ubicación geocodificada del usuario
        city_name = user_location.city_name;
    ...
    ESP_LOGI(TAG, "Solicitando clima para %s: %s", city_name, url);
    ...
    esp_http_client_config_t config = {
        ...
        .is_async = false,
        .timeout_ms = 5000,  // Reduce timeout from 10s to 5s to prevent watchdog
    };
    ...
    ESP_LOGI(TAG, "Starting HTTP request for weather data...");
    esp_err_t err = esp_http_client_perform(client);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Fallo en solicitud HTTP: %s", esp_err_to_name(err));
    } else {
    ...
    ESP_LOGI(TAG, "HTTP request completed with result: %s", esp_err_to_name(err));
    return err;
}
...
{
    return user_location.is_valid ? &user_location : NULL;
}

// Configurar ubicación de usuario desde resultado de geocoding
esp_err_t app_weather_set_custom_location(const geocoding_location_t* location)
{
    if (!location || !location->is_valid) {
    ESP_LOGE(TAG, "Ubicación inválida para ubicación de usuario");
        return ESP_ERR_INVALID_ARG;
    }
    
    memcpy(&user_location, location, sizeof(geocoding_location_t));
    ESP_LOGI(TAG, "Ubicación usuario configurada: %s (%.4f, %.4f)", 
             user_location.city_name, user_location.latitude, user_location.longitude);
    return ESP_OK;
}
...
{
    ESP_LOGI(TAG, "Inicializando módulo de clima con Open-Meteo API");
    memset(weather_data, 0, sizeof(weather_data));
    memset(&user_location, 0, sizeof(user_location));
    return ESP_OK;
}
...
esp_err_t app_weather_parse_geocoding(const char *json_string, geocoding_location_t* result)
{
    cJSON *json = cJSON_Parse(json_string);
    if (json == NULL) {
        ESP_LOGE(TAG, "Error al parsear JSON de geocodificación");
        return ESP_ERR_INVALID_ARG;
    }

    cJSON *results = cJSON_GetObjectItem(json, "results");
    if (results == NULL || !cJSON_IsArray(results)) {
        ESP_LOGE(TAG, "Sin resultados en respuesta de geocodificación");
        cJSON_Delete(json);
    ...
    if (array_size == 0) {
        ESP_LOGE(TAG, "Arreglo de resultados vacío en geocodificación");
        cJSON_Delete(json);
        return ESP_ERR_NOT_FOUND;
    }
    ...
    if (first_result == NULL) {
        ESP_LOGE(TAG, "Error al obtener primer resultado de geocodificación");
        cJSON_Delete(json);
        return ESP_ERR_INVALID_ARG;
    }
    ...
    if (name && cJSON_IsString(name) && latitude && cJSON_IsNumber(latitude) && longitude && cJSON_IsNumber(longitude)) {
        ...
        ESP_LOGI(TAG, "Geocodificación encontró: %s (%.4f, %.4f)", 
                 result->city_name, result->latitude, result->longitude);
    } else {
        ESP_LOGE(TAG, "Formato de resultado de geocodificación inválido");
        cJSON_Delete(json);
        return ESP_ERR_INVALID_ARG;
    }
...
esp_err_t http_geocoding_event_handler(esp_http_client_event_t *evt)
{
    ...
    switch (evt->event_id) {
    case HTTP_EVENT_ERROR:
        ESP_LOGD(TAG, "HTTP_EVENT_ERROR (geocodificación)");
        break;
    case HTTP_EVENT_ON_CONNECTED:
        ESP_LOGD(TAG, "HTTP_EVENT_ON_CONNECTED (geocodificación)");
        break;
    case HTTP_EVENT_HEADER_SENT:
        ESP_LOGD(TAG, "HTTP_EVENT_HEADER_SENT (geocodificación)");
        break;
    case HTTP_EVENT_ON_HEADER:
        ESP_LOGD(TAG, "HTTP_EVENT_ON_HEADER (geocodificación), key=%s, value=%s", evt->header_key, evt->header_value);
        break;
    case HTTP_EVENT_ON_DATA:
        ESP_LOGD(TAG, "HTTP_EVENT_ON_DATA (geocodificación), len=%d", evt->data_len);
        // Store received data
        if (output_buffer == NULL) {
            output_buffer = (char *) malloc(MAX_HTTP_RECV_BUFFER);
            output_len = 0;
        ...
        break;
    case HTTP_EVENT_ON_FINISH:
        ESP_LOGD(TAG, "HTTP_EVENT_ON_FINISH (geocodificación)");
        if (output_buffer != NULL) {
        ...
        break;
    case HTTP_EVENT_DISCONNECTED:
        ESP_LOGD(TAG, "HTTP_EVENT_DISCONNECTED (geocodificación)");
        if (output_buffer != NULL) {
            free(output_buffer);
            output_buffer = NULL;
            output_len = 0;
                                          ...
                                          int max_results, int* num_results)
{
    *num_results = 0;
    ...
    if (!json_data || !results) {
        ESP_LOGE(TAG, "Parámetros inválidos para parsear respuesta de ciudades");
        return;
    }
    ...
    while (*current && result_count < max_results) {
        ...
        if (result->is_valid) {
            ESP_LOGI(TAG, "Ciudad parseada %d: %s (%s) - %.4f, %.4f", 
    ...
    *num_results = result_count;
    ESP_LOGI(TAG, "Se parsearon %d ciudades desde respuesta de geocodificación", result_count);
}
...
{
    static const char *TAG_HTTP = "geocod_ciudades_http";
    ...
    switch(evt->event_id) {
        ...
        case HTTP_EVENT_ON_FINISH:
            ...
            if (search_data->response_len > 0) {
                ESP_LOGI(TAG_HTTP, "Respuesta geocodificación: %s", search_data->response_buffer);
                                             ...
                                             geocoding_location_t* results, int max_results, int* num_results)
{
    if (!country_code || !city_name || !results || !num_results || max_results <= 0) {
        ESP_LOGE(TAG, "Parámetros inválidos para búsqueda de geocodificación de ciudades");
        return ESP_ERR_INVALID_ARG;
    }
    ...
    *num_results = 0;
    ...
    ESP_LOGI(TAG, "URL geocodificación ciudades: %s", url);
    ...
    esp_http_client_handle_t client = esp_http_client_init(&config);
    if (client == NULL) {
        ESP_LOGE(TAG, "Error al inicializar cliente HTTP para geocodificación de ciudades");
        return ESP_ERR_NO_MEM;
    }

    esp_err_t err = esp_http_client_perform(client);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Fallo en solicitud HTTP geocodificación ciudades: %s", esp_err_to_name(err));
    } else {
        int status_code = esp_http_client_get_status_code(client);
        ESP_LOGI(TAG, "Estado HTTP geocodificación ciudades = %d, %d resultados", status_code, *num_results);
    }
